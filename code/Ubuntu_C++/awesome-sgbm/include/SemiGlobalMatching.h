...
public:
	...
	/**
	 * \brief Get the census result of left and right image
	 * \param census_left		输入，get census result of left eye
	 * \param census_right	        输入，get census result of right eye
	 */
        bool GetCensus(uint32* census_left, uint32* census_right);
        
        /**
	 * \brief Get the origin cost with hamming distance
	 * \param cost_init		输入，get origin cost
	 */
        bool GetOriginCost(uint8* cost_init);
...
private:
	...
	/** \brief 视差计算	 */
	void ComputeDisparity_init() const;
